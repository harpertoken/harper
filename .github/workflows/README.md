...
| Nix and Arch Packages | `nix-arch-packages.yml` | Generates Nix and Arch package manifests from published Linux release artifacts and uploads them for package-channel validation. | Manual dispatch |
| npm Package | `npm-package.yml` | Packs the npm binary wrapper for a published Harper release artifact without publishing it. | Manual dispatch |
